...
public class GraphAlgorithm implements ReferringExpressionAlgorithm {
    ...
    public GraphAlgorithm(Map<String, List<String>> priorities,
	...
	DirectedPseudograph<Resource, Edge> candidate = new DirectedPseudograph<Resource, Edge>(
		Edge.class);
	candidate.addVertex(referent);

	DirectedPseudograph<Resource, Edge> finalGraph = findGraph(referent,
		graph, bestGraph, Double.NaN, candidate).getLeft();
	...
	for (Edge e : finalGraph.edgeSet())
	    statements.add(repo
		    .getStatements(
			    finalGraph.getEdgeSource(e),
			    e.getURI(),
			    e.isRelation() ? finalGraph.getEdgeTarget(e) : e
				    .getValue(), true).next());

	return new Result(statements);
    }

    private DirectedPseudograph<Resource, Edge> buildGraph(
	    RepositoryConnection repo, URI[] uris) throws RepositoryException {
	DirectedPseudograph<Resource, Edge> result = new DirectedPseudograph<Resource, Edge>(
		Edge.class);

	Set<Statement> consumed = new HashSet<Statement>();

	for (URI uri : uris) {
	    buildGraphAddAll(result, repo.getStatements(uri, null, null, true),
		    consumed);
	    buildGraphAddAll(result, repo.getStatements(null, null, uri, true),
		    consumed);
	}
    ...
    private void buildGraphAddAll(DirectedPseudograph<Resource, Edge> graph,
	    RepositoryResult<Statement> stmts, Set<Statement> consumed)
    ...
    private Pair<DirectedPseudograph<Resource, Edge>, Double> findGraph(
	    URI referent, DirectedPseudograph<Resource, Edge> fullGraph,
	    DirectedPseudograph<Resource, Edge> bestGraph,
	...
	List<Resource> distractors = new ArrayList<Resource>(fullGraph
		.vertexSet().size());
	...
	for (Edge e : neighbors(candidate, fullGraph)) {
	    ...
	    newCandidate.addEdge(fullGraph.getEdgeSource(e),
		    fullGraph.getEdgeTarget(e), e);
	    Pair<DirectedPseudograph<Resource, Edge>, Double> p = findGraph(
		    referent, fullGraph, bestGraph, bestGraphCost, newCandidate);
    ...
    private Set<Resource> neighbors(DirectedPseudograph<Resource, Edge> graph,
	    Resource node) {
    ...
    private boolean containedPropertiesInSubgraph(Resource node1,
	    DirectedPseudograph<Resource, Edge> subGraph, Resource node2,
	    DirectedPseudograph<Resource, Edge> fullGraph) {
    ...
    private boolean matchGraphs(Resource referent,
	    DirectedPseudograph<Resource, Edge> candidate, Resource other,
	    DirectedPseudograph<Resource, Edge> fullGraph) {

	if (!containedPropertiesInSubgraph(referent, candidate, other,
		fullGraph))
	    return false;
	Map<Resource, Resource> bijection = new HashMap<>();
	bijection.put(referent, other);

	return matchHelper(bijection, neighbors(candidate, referent),
		fullGraph, candidate);
    }
	...
	for (Resource toMap : neighborgs) { // y
		...
		if (bijection.values().contains(extension))
		    continue;
		if (!containedPropertiesInSubgraph(toMap, candidate, extension,
			fullGraph))
		    continue;
		...
		for (Resource other : toCheck) { // h
		    ...
		    if (!fullGraph.getAllEdges(extension, bijection.get(other))
			    .containsAll(outgoing)) {
			good = false;
			break;
		    }
		    // (other -> toMap) y->h
		    Set<Edge> incoming = candidate.getAllEdges(other, toMap);
		    if (!fullGraph.getAllEdges(bijection.get(other), extension)
			    .containsAll(incoming)) {
			good = false;
			break;
		    }
		}
		...
		if (matchHelper(bijectionRec, neighborgsRec, fullGraph,
			candidate))
		    return true;
	    }
